...
> 当前版本为 `0.0.3`，生产协议 v2 的业务传输链路已经接通；仍未包含 Push/APNs、用户注册、账户限流和套餐计费。发布前还需要完成真机验收与平台签名配置。
...
- [Actent](#actent)
  ...
  - [ntfy 服务器](#ntfy-服务器)
  - [代码职责](#代码职责)
    - [Dartloom 负责](#dartloom-负责)
    - [Actent 负责](#actent-负责)
  - [生产 v2 实现状态](#生产-v2-实现状态)
    - [1. 重构通用传输层](#1-重构通用传输层)
    - [2. 接通生产 ntfy](#2-接通生产-ntfy)
    - [3. 完成加密附件和断点续传](#3-完成加密附件和断点续传)
    - [4. 完成可靠投递](#4-完成可靠投递)
    - [5. 收紧配对和密钥边界](#5-收紧配对和密钥边界)
    - [6. 完成应用端生产配置](#6-完成应用端生产配置)
    - [7. 验收和运维](#7-验收和运维)
