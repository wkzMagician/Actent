...
Pigeon is a cross-device programmable content router:

```text
Android Share -> PigeonMessage -> Work Picker -> LAN-first encrypted Packet
...
The Pigeon layer only owns `PigeonMessage`, Work, Catalog, Inbox, persistence
...
`schemas/pigeon_message.schema.json` (schema version 1).
...
`content://` attachments are copied into Pigeon's private attachment directory
before a versioned `PigeonMessage` is emitted. FileProvider grants temporary
...
server URL and authorization value in Secure Settings. Restart Pigeon after a
...
```text
.\tool\run_windows.ps1 -FlutterArgs '--dart-define=PIGEON_LAN_CERT_PATH=C:/path/pigeon.crt' '--dart-define=PIGEON_LAN_KEY_PATH=C:/path/pigeon.key' '--dart-define=PIGEON_LAN_HOST=192.168.1.20'
```
