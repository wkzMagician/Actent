...
enum MdnsDiscoveryStatus { idle, completed, networkConfigurationProblem }
...
class MdnsPairingDiscovery implements PairingDiscovery {
  ...
  MdnsDiscoveryStatus _lastStatus = MdnsDiscoveryStatus.idle;

  MdnsDiscoveryStatus get lastStatus => _lastStatus;

  @override
  Stream<PairingAdvertisement> discover() async* {
    _lastStatus = MdnsDiscoveryStatus.idle;
    ...
    try {
      ...
      final usableInterfaces = interfaces.where(
        (interface) => !_isLikelyTunnel(interface.name),
      );
      if (usableInterfaces.isEmpty) {
        _lastStatus = MdnsDiscoveryStatus.networkConfigurationProblem;
        return;
      }
      await client.start(
        interfacesFactory: (_) async => usableInterfaces.where(
      ...
      // Windows can reject multicast socket options on virtual or disabled
      // adapters with WSAEINVAL (10042). Treat mDNS as unavailable instead
      // of exposing a platform error to users; pairing can still use QR or
      // the relay path.
      _lastStatus = MdnsDiscoveryStatus.networkConfigurationProblem;
      return;
    }
    _lastStatus = MdnsDiscoveryStatus.completed;
    try {
  ...
  static bool _isLikelyTunnel(String name) {
